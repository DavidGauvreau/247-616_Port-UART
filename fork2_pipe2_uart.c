...
void main(void)
{
    pipe(pipe1); // père → fils
    pipe(pipe3); // fils → père
    ...
    if (pid1 == 0)
    {
        close(pipe1[1]); // on lit pipe1
        close(pipe3[0]); // on écrit pipe2
        ...
        while (1)
        {
            ...
            {
                const char *message = "Salut père, j'ai terminé ! De fils 1";
        ...
        //on ferme les pipes qu'on n'utilise plus.
        close(pipe1[0]);
        close(pipe3[1]);
    ...
    else
    {
        ...
        if (pid2 == 0)
        {
            //on utilise aucun pipe dans le fils 2
            close(pipe1[0]);
            close(pipe3[0]);
            close(pipe1[1]);
            close(pipe3[1]);
        ...
        {
            close(pipe1[0]); // on écrit pipe1
            close(pipe3[1]); // on lit pipe2
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    char reply[100];
                    read(pipe3[0], reply, sizeof(reply));
